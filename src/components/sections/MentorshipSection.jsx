...
const MentorshipSection = () => {
	...
	const subteams = subteamData.map((team) => ({
		...team,
		icon: iconMap[team.icon]
			? React.createElement(iconMap[team.icon], { className: "w-10 h-10" })
			: null,
	}));
	...
	const renderContentBoxes = (landscapeMode = false) => (
		<div className="flex flex-col gap-4">
			...
			<motion.div
				className={`flex ${
					landscapeMode ? "gap-6" : "flex-col sm:flex-row gap-4 sm:gap-6"
				}`}
	...
	return (
		<div className=" flex flex-col pt-2 pb-4 sm:pt-3  sm:pb-6 sm:px-12 lg:pt-4 lg:pb-8 lg:px-8">
						...
						>
							<div className="space-y-6 text-sm leading-relaxed">
								<p>
									...
									it's important to have an expert to provide young minds with
									new perspectives and new skills that were never taught before.
									Mentors fulfill that need for an "expert", and coming in only
									for a few hours a week can be enough to get students inspired
									and motivated for robotics.
								</p>
								<p>
									To the right is an interactive chart of the subteams in which
									our team operates{" "}
									...
									listed, then feel free to email us about your inquiry. Even if
									you are not an expert in the topics provided, simply a new
									perspective can go a long way in our success.
								</p>
							</div>
						</motion.div>
						{/* Right Section */}
			...
			>
				<h2
					className="text-lg sm:text-lg font-bold tracking-[0.2em] text-white mb-8"
					style={{ fontFamily: "HK Modular, sans-serif" }}
				>
